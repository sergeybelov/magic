...
optimal_n=np.intersect1d(np.argwhere(var1>75.),np.argwhere(svd.explained_variance_ratio_<=10**-3))[0]
print(optimal_n)#171
...
def getPlt(start, finish, folds):
    ...
    print('GaussianMixture starts.')
    for _min_cluster_size in np.arange(start,finish,1):    
        print('min_cluster_size=',_min_cluster_size)
            
        hdb_t1 = time.time()

        gm=GaussianMixture(n_components=_min_cluster_size,max_iter=150,n_init=3,random_state=17).fit(svd_representation)
        hdb_elapsed_time = time.time() - hdb_t1
        print('GaussianMixture Elapsed time to cluster: %4.1f m' % (hdb_elapsed_time/60))            
        print('aic=',gm.aic(svd_representation))
                
        for index in subs:
            hdb_t1 = time.time()
            cluster_labels=gm.predict(svd_representation[index[0]])
            ...
            ax1.set_ylim([0, len(X) + (_min_cluster_size + 1) * 13])
            ...
            print("For n_clusters =", _min_cluster_size,
            ...
            y_lower = 12
            for i in range(_min_cluster_size):
                ...
                color = cm.spectral(float(i) / _min_cluster_size)
                ...
                y_lower = y_upper + 13  # 10 for the 0 samples
            ...
            plt.suptitle(("Silhouette analysis for HDBSCAN clustering on sample data "
                          "with n_clusters = %d" % _min_cluster_size),
                         fontsize=14, fontweight='bold')
    
            plt.show()
            
        print(_min_cluster_size)
        print('Next')    
